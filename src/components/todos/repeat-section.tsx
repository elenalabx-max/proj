...
export function RepeatSection({ todo }: { todo: Todo }) {
  ...
  const { data: rule } = useRule(todo.recurrence_rule_id);

  const [freq, setFreq] = useState<RecurrencePattern["freq"]>("weekly");
  const [weekdays, setWeekdays] = useState<number[]>([]);
  const [monthDay, setMonthDay] = useState(1);
  const [startsOn, setStartsOn] = useState(todo.date ?? todayISODate());
  ...
  function handleSave() {
    ...
    setRecurrence.mutate({ todoId: todo.id, pattern, startsOn, endsOn: endsOn || null });
    setEditing(false);
  }
